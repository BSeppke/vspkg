...
#------------------------------------------------------------------------------
unpack-file "..\opencv-2.4.11-patch.zip" "$VSP_INSTALL_PATH"  >> $logFile
...
&"$VSP_BIN_PATH\cmake.exe" "-G$VSP_CMAKE_MSVC_GENERATOR" "-Wno-dev" "-DCMAKE_INSTALL_PREFIX=$VSP_INSTALL_PATH" "-DCMAKE_PREFIX_PATH=$VSP_INSTALL_PATH" "-DCMAKE_BUILD_TYPE=Release" "-DWITH_OPENEXR=1" "-DWITH_QT=1" "-DWITH_JPEG=1" "-DWITH_PNG=1" "-DWITH_TIFF=1" "-DBUILD_opencv_python=1" "-DBUILD_SHARED_LIBS=1" ".." >> $logFile
...
cp "..\..\opencv-2.4.11-patch\share\cmake\opencv\*.cmake" "$VSP_SHARE_PATH\cmake\opencv" -force
cp "..\..\opencv-2.4.11-patch\lib\*.cmake" "$VSP_LIB_PATH"  -force
