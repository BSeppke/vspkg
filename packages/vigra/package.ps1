...
#VIGRA does not find boost_python, so help a bit:
$BOOST_PYTHON_LIB = Get-ChildItem "$VSP_LIB_PATH\boost_python*.lib"
...
{
	&"$VSP_BIN_PATH\cmake.exe" "-G$VSP_CMAKE_MSVC_GENERATOR" "-DCMAKE_INSTALL_PREFIX=$VSP_INSTALL_PATH" "-DCMAKE_PREFIX_PATH=$VSP_INSTALL_PATH" "-DWITH_OPENEXR=1" "-DDEPENDENCY_SEARCH_PREFIX=$VSP_INSTALL_PATH" "-DZLIB_LIBRARY=$VSP_LIB_UNIXPATH/z.lib" "-DJPEG_LIBRARY=$VSP_LIB_UNIXPATH/jpeg.lib"  "-DTIFF_LIBRARY=$VSP_LIB_UNIXPATH/tiff.lib" "-DPNG_LIBRARY=$VSP_LIB_UNIXPATH/png.lib" "-DWITH_VIGRANUMPY=1" "-DBoost_PYTHON_LIBRARY=$BOOST_PYTHON_LIB" ".." >> $logFile
}
